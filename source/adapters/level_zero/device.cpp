...
ur_result_t urDeviceGetInfo(
  ...
  case UR_DEVICE_INFO_COMMAND_BUFFER_SUPPORT_EXP:
    ...
    //    // Update support requires being able to update kernel arguments and
    //    all
  ...
  case UR_DEVICE_INFO_COMMAND_BUFFER_UPDATE_CAPABILITIES_EXP: {
    const bool ZeMutableCommandFlags =
        Device->ZeDeviceMutableCmdListsProperties->mutableCommandFlags;

    ur_device_command_buffer_update_capability_flags_t UpdateCapabilities = 0;
    if (ZeMutableCommandFlags & ZE_MUTABLE_COMMAND_EXP_FLAG_KERNEL_ARGUMENTS) {
      UpdateCapabilities |=
          UR_DEVICE_COMMAND_BUFFER_UPDATE_CAPABILITY_FLAG_KERNEL_ARGUMENTS;
    }
    ze_mutable_command_exp_flags_t ReqUpdateWG =
        ZE_MUTABLE_COMMAND_EXP_FLAG_GROUP_COUNT |
        ZE_MUTABLE_COMMAND_EXP_FLAG_GROUP_SIZE;
    if ((ZeMutableCommandFlags & ReqUpdateWG) == ReqUpdateWG) {
      UpdateCapabilities |=
          UR_DEVICE_COMMAND_BUFFER_UPDATE_CAPABILITY_FLAG_GLOBAL_WORK_SIZE |
          UR_DEVICE_COMMAND_BUFFER_UPDATE_CAPABILITY_FLAG_LOCAL_WORK_SIZE;
    }
    if (ZeMutableCommandFlags & ZE_MUTABLE_COMMAND_EXP_FLAG_GLOBAL_OFFSET) {
      UpdateCapabilities |=
