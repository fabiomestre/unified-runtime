...
struct ur_exp_command_buffer_handle_t_ : public _ur_object {
  ...
  // Level Zero command list handle that has the compute engine command for this command-buffer.
  ...
  // Level Zero command list handle that has the copy engine command for this command-buffer.
  ...
  // [WaitEvent Path Only] Event which a command-buffer waits on until the wait-list
  // dependencies passed to a command-buffer enqueue have been satisfied.
  ...
  // [ImmediateAppend Path Only] Event that is signalled after the compute engine
  // command-list finishes executing.
  ur_event_handle_t ComputeFinishedEvent = nullptr;
  // [ImmediateAppend Path Only] Event that is signaled after the current submission
  // of this command-buffer finishes executing (i.e. after ZeComputeCommandList
  // finishes executing).
  ...
  // [WaitEvent Path only] Level Zero fences for each queue the command-buffer has been
  // enqueued to. These should be destroyed when the command-buffer is released.
  std::unordered_map<ze_command_queue_handle_t, ze_fence_handle_t> ZeFencesMap;
  // [WaitEvent Path only] The Level Zero fence from the most recent enqueue of the
  // command-buffer. Must be an element in ZeFencesMap, so is not required to
  // be destroyed itself.
